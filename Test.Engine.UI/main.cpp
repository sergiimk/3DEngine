...
VOID Render()
{
	...
	g_layer1->setToleranceToAll(GetTolerance(g_cameraPos.length()));
	g_layer1->RecalculateAll();

	g_layer2->setToleranceToAll(GetTolerance(g_cameraPos.length()));
	g_layer2->RecalculateAll();

	if(pRenderer->BeginFrame())
	{
		g_layer1->Render();

		world->Push(VML::Matrix4::translation(VML::Vector3(0, 0, -20)));
