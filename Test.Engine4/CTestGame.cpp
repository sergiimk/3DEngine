...
void CTestGame::Initialize(Engine::IMainLoop *pMainLoop)
{
	...
	// Create blurred SSAO buffer
	sdesc.Format = FMT_L8;
	gEnv->SurfaceManager->CreateNewTexture(sdesc, buf_ssao_blured[0].wrapped());
	gEnv->SurfaceManager->CreateNewTexture(sdesc, buf_ssao_blured[1].wrapped());
	buf_ssao_blured[0]->AddUser();
	buf_ssao_blured[1]->AddUser();


	// Create down sampled SSAO buffer
	//sdesc.Width >>= 1;
	//sdesc.Height >>= 1;
	gEnv->SurfaceManager->CreateNewTexture(sdesc, buf_ssao.wrapped());
	buf_ssao->AddUser();

	gEnv->SurfaceManager->FindOrLoadTexture(Utils::URL("SSAO_noise.bmp"), tex_noise.wrapped());
	...
	gEnv->EffectManager->FindOrLoadEffect(Utils::URL("ssao_compute.fxd"), &pEffect);
	gEnv->MaterialManager->CreateMaterial(mat_ssao.wrapped());
	bindings = mat_ssao->GetBindings();
	bindings->addTextureBinding(FXP_TEXTURE_DEPTH, gbuf_depth);
	bindings->addTextureBinding(FXP_TEXTURE_NORMAL, gbuf_normal);
	bindings->addTextureBinding(FXP_TEXTURE_SHININESS, tex_noise);
	mat_ssao->SetEffect(0, pEffect);
	pEffect->Release();
	pEffect = 0;

	// Create SSAO bilateral blur material
	gEnv->EffectManager->FindOrLoadEffect(Utils::URL("ssao_blur.fxd"), &pEffect);
	gEnv->MaterialManager->CreateMaterial(mat_ssao_blur.wrapped());
	mat_ssao_blur->SetEffect(0, pEffect);
	pEffect->Release();
	pEffect = 0;

	mat_accumulation->AddUser();
	mat_ssao->AddUser();
	mat_ssao_blur->AddUser();
...
CTestGame::~CTestGame()
{
	...
	mat_ssao->RemoveUser();
	mat_ssao_blur->RemoveUser();

	gbuf_depth->RemoveUser();
	gbuf_normal->RemoveUser();
	acc_lighting->RemoveUser();
	buf_ssao->RemoveUser();
	buf_ssao_blured[0]->RemoveUser();
	buf_ssao_blured[1]->RemoveUser();
...
void CTestGame::SetupScene()
{
	...
	//////////////////////////////////////////////////////////////////////////
	// Terrain model
	gEnv->GeometryManager->FindOrLoadMesh(Utils::URL("sibenik_cathedral.md"),  terrain_model.wrapped());
...
void CTestGame::Render(double frameDelta)
{
	...
	if(gEnv->Renderer->BeginFrame())
	{
		...
		// Compute initial SSAO
		gEnv->Renderer->SetRenderTarget(0, buf_ssao->GetRenderTarget());
		gEnv->Renderer->SetMaterial(mat_ssao);
		DrawFSQuad();

		// Blur SSAO buffer (X-axis)
		SMaterialBindings* bindings = mat_ssao_blur->GetBindings();
		bindings->addTextureBinding(FXP_TEXTURE_SSAO, buf_ssao);
		gEnv->Renderer->SetRenderTarget(0, buf_ssao_blured[0]->GetRenderTarget());
		gEnv->Renderer->SetMaterial(mat_ssao_blur);
		DrawFSQuad(0);

		// Y-axis
		bindings->addTextureBinding(FXP_TEXTURE_SSAO, buf_ssao_blured[0]);
		gEnv->Renderer->SetRenderTarget(0, buf_ssao_blured[1]->GetRenderTarget());
		gEnv->Renderer->SetMaterial(mat_ssao_blur);
		DrawFSQuad(1);
