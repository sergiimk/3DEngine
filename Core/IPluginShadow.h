...
namespace Core
{
	 ...
	 *  
	 *  @ingroup Core */
	SCOM_INTERFACE(IPluginShadow, "b8e5b63a-4b08-4dcf-8fa2-9e0a54379c37", SCOM::IUnknown)
	{
	public:
		/// Gets plug-in name
		...
		virtual bool Exports(const SCOM::GUID &clsid) = 0;
		...
		virtual SCOM::HResult CreateInstance(SCOM_RIID clsid, SCOM_RIID riid, void** ppv) = 0;
		/// Queries the creation of first found interface implementation (loads module automatically)
		virtual SCOM::HResult CreateInstance(SCOM_RIID riid, void** ppv) = 0;
